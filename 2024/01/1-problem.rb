...
# fifty places they'll look, so in order to save Christmas, you need to help them get fifty stars on their list before Santa takes off 
# on December 25th.
...
# To make sure they don't miss anything, The Historians split into two groups, each searching the office and trying to create their own 
...
# Flatten the array of arrays and sort ascending
left_list_array = temp_left.flatten.sort
right_list_array = temp_right.flatten.sort

# Calculate difference of values at array index and take absolute value
# Shove into new array of differences
while (index <= left_list_array.length)
  difference_array.push((left_list_array[index].to_i - right_list_array[index].to_i).abs)
  index+=1
end

# Sum all values of array
puts difference_array.sum
