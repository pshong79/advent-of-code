...
def calulate_power(arr)
  ...
  arr.map do |line| 
    ...
    green_arr.empty? ? green_max = 1 : green_max = green_arr.max
    blue_arr.empty? ? blue_max = 1 : blue_max = blue_arr.max
    red_arr.empty? ? red_max = 1 : red_max = red_arr.max

    power = green_max * blue_max * red_max
    power_arr << power
  end
  power_sum = power_arr.sum
end

## Run solution. Answer = 79315
split_into_array = read_input("input.txt")
power_sum = calulate_power(split_into_array)
puts "power_sum = #{power_sum}"
